...
void chooseSeat(FILE *file) {
    ...
    int maxAttempts = totalSeats * 2; // Set a maximum number of attempts
    ...
    while (attempts < maxAttempts) {
        ...
        if (accepted == 1) {
            ...
            // tu je problem jer oba su potvrdila mjesto
            // ali predugo server nema neku autentikaciju 
            // jeli mjesto vec zauzeto
            seating[row][col] = 1;
            printf("NADENO SJEDALO [%d][%d]\n", row, col);
            fprintf(file, "NADENO SJEDALO [%d][%d]\n", row, col); // Write to the file
            return; // Exit the function after finding and reserving a seat
        }
        attempts++;
    }
